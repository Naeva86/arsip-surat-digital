...
<div class="flex items-center justify-between mb-5">
    <div>
        <h2 class="text-lg font-semibold text-gray-800 dark:text-white">Surat Masuk</h2>
        <p class="text-sm text-gray-400 mt-0.5">Kelola semua surat masuk</p>
    </div>
    <div class="flex items-center gap-2">
        ...
        <button onclick="toggleFilter()"
            ...
            Filter
           @php $activeCount = count(array_filter(request()->only(['status','sifat','kategori_id','dari_tanggal','sampai_tanggal']))); @endphp
            @if($activeCount > 0)
...
<div class="flex flex-wrap items-center gap-2 mb-4">
    ...
    <span class="inline-flex items-center gap-1.5 text-xs bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2.5 py-1 rounded-full">
        ...
        <a href="{{ request()->fullUrlWithQuery(['status' => null]) }}" class="hover:opacity-70 font-bold">x</a>
    </span>
    @endif
    @if(request('sifat'))
    <span class="inline-flex items-center gap-1.5 text-xs bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2.5 py-1 rounded-full">
        Sifat: {{ ucfirst(request('sifat')) }}
        <a href="{{ request()->fullUrlWithQuery(['sifat' => null]) }}" class="hover:opacity-70 font-bold">x</a>
    </span>
    @endif
    @if(request('dari_tanggal') || request('sampai_tanggal'))
    <span class="inline-flex items-center gap-1.5 text-xs bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2.5 py-1 rounded-full">
        {{ request('dari_tanggal') }} - {{ request('sampai_tanggal') }}
        <a href="{{ request()->fullUrlWithQuery(['dari_tanggal' => null, 'sampai_tanggal' => null]) }}" class="hover:opacity-70 font-bold">x</a>
    </span>
    @endif
    <a href="{{ route('surat-masuk.index') }}" class="text-xs text-red-500 hover:text-red-700 hover:underline ml-1">Hapus semua</a>
</div>
@endif
...
<div id="filter-panel"
    ...
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700 flex-shrink-0">
        <div>
            <h3 class="font-semibold text-gray-800 dark:text-white text-sm">Filter Surat Masuk</h3>
    ...
    <form method="GET" action="{{ route('surat-masuk.index') }}" class="flex flex-col flex-1 overflow-hidden">
        <div class="flex-1 overflow-y-auto px-5 py-5 space-y-5">
            ...
            <div>
                ...
                <div class="space-y-1.5">
                    @foreach(['' => 'Semua Status', 'baru' => 'Baru', 'menunggu_direktur' => 'Menunggu Direktur', 'ditolak' => 'Ditolak', 'proses_disposisi' => 'Proses Disposisi', 'selesai' => 'Selesai'] as $val => $label)
                    <label class="filter-radio-label flex items-center gap-3 px-3 py-2.5 border rounded-lg cursor-pointer transition {{ request('status') === $val ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 dark:border-blue-500' : 'border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700/50' }}">
                        <input type="radio" name="status" value="{{ $val }}" {{ request('status') === $val ? 'checked' : '' }} class="hidden">
                        <span class="w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 {{ request('status') === $val ? 'border-blue-500' : 'border-gray-300 dark:border-gray-500' }}">
                            @if(request('status') === $val)
                            <span class="w-2 h-2 rounded-full bg-blue-500"></span>
                            @endif
                        </span>
                        <span class="text-sm {{ request('status') === $val ? 'text-blue-600 dark:text-blue-400 font-medium' : 'text-gray-600 dark:text-gray-300' }}">{{ $label }}</span>
                    </label>
                    @endforeach
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-1.5">
                    @foreach(['' => 'Semua', 'biasa' => 'Biasa', 'penting' => 'Penting', 'rahasia' => 'Rahasia', 'urgent' => 'Urgent'] as $val => $label)
                    <label class="filter-radio-label flex items-center justify-center gap-2 px-3 py-2 border rounded-lg cursor-pointer transition text-center {{ request('sifat') === $val ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 dark:border-blue-500' : 'border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700/50' }}">
                        <input type="radio" name="sifat" value="{{ $val }}" {{ request('sifat') === $val ? 'checked' : '' }} class="hidden">
                        <span class="text-xs {{ request('sifat') === $val ? 'text-blue-600 dark:text-blue-400 font-medium' : 'text-gray-600 dark:text-gray-300' }}">{{ $label }}</span>
                    </label>
                    @endforeach
                </div>
            </div>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                ...
                @php
                    ...
                    $statusColor = match($surat->status) {
                        'baru'              => 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400',
                        'menunggu_direktur' => 'bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400',
                        'ditolak'           => 'bg-red-50 text-red-600 dark:bg-red-900/30 dark:text-red-400',
                        'proses_disposisi'  => 'bg-yellow-50 text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-400',
                    ...
                    $statusColorModal = match($surat->status) {
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                    ...
                    <td class="px-4 py-3">
                        ...
                        @if($surat->status === 'ditolak')
                            @php $penolakan = $surat->disposisis->where('keputusan', 'ditolak')->last(); @endphp
                            @if($penolakan && $penolakan->catatan_penolakan)
                            <div class="text-xs text-red-500 mt-1 truncate max-w-[150px]" title="{{ $penolakan->catatan_penolakan }}">
                                💬 {{ Str::limit($penolakan->catatan_penolakan, 30) }}
                            </div>
                            @endif
                        @endif
                    </td>
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-center gap-1">
                            ...
                            {{-- Cetak Disposisi — hanya saat selesai --}}
                            @if($surat->status === 'selesai' && $surat->disposisis->count() > 0)
                            @php $lastDispo = $surat->disposisis->sortByDesc('created_at')->first(); @endphp
                            <a href="{{ route('disposisi.cetak', $lastDispo) }}" target="_blank" title="Cetak Disposisi"
                            ...
                            {{-- Edit & Hapus — hanya saat status ditolak (semua role termasuk admin) --}}
                            @if($surat->status === 'ditolak')
                            <a href="{{ route('surat-masuk.edit', $surat) }}" title="Edit"
                            ...
                            <form action="{{ route('surat-masuk.destroy', $surat) }}" method="POST" class="inline"
                                data-konfirmasi data-tipe="hapus" data-judul="Hapus Surat?" data-pesan="Surat ini akan dihapus permanen." data-label-ok="Ya, Hapus">
                                @csrf @method('DELETE')
                ...
                {{-- ═══════════════════════════════════════ --}}
                {{-- MODAL DETAIL INLINE — pure Blade only   --}}
                {{-- Tidak ada JavaScript data mapping sama sekali --}}
                {{-- ═══════════════════════════════════════ --}}
                <tr>
                                ...
                                <div class="flex-1 overflow-y-auto px-6 py-5">
                                    <div class="grid grid-cols-2 gap-x-8 gap-y-4 text-sm">
                                        ...
                                        <div>
                                            ...
                                            <span class="text-xs px-2.5 py-1 rounded-full font-medium {{ $statusColorModal }}">
                                                {{ $surat->status_label }}
                                            </span>
                                        </div>
                                        <div>
                                        ...
                                        <div class="col-span-2">
                                            ...
                                            <p class="text-gray-700 dark:text-gray-300 text-sm">{{ $surat->keterangan ?? '-' }}</p>
                                        </div>
                                    </div>
                                </div>
    ...
    radio.addEventListener('change', function() {
        ...
        document.querySelectorAll('.filter-radio-label input[name="' + name + '"]').forEach(function(r) {
            ...
            if (r.checked) {
                ...
                if (span) { span.classList.add('text-blue-600','font-medium'); span.classList.remove('text-gray-600'); }
            } else {
                lbl.classList.remove('border-blue-500','bg-blue-50');
                lbl.classList.add('border-gray-200');
                if (span) { span.classList.remove('text-blue-600','font-medium'); span.classList.add('text-gray-600'); }
            }
        });
    });
});
